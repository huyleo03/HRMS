...
/**
 * GET /api/dashboard/stats/department-comparison
 * So sánh các phòng ban về attendance, requests, payroll
 * @access Admin only
 */
exports.getDepartmentComparison = async (req, res) => {
  try {
    ...
    // Parallel queries cho mỗi department
    const comparisonData = await Promise.all(
      departments.map(async (dept) => {
        ...
        const attendanceStats = await Attendance.aggregate([
          ...
          {
            $match: {
              "user.department.department_id": deptId,
              date: { $gte: startOfMonth, $lte: endOfMonth },
            },
          },
          {
        ...
        const avgWorkHours =
          attendanceStats.length > 0 && attendanceStats[0].present > 0
            ? parseFloat(
                (attendanceStats[0].totalWorkHours / attendanceStats[0].present).toFixed(1)
              )
            : 0;

        // 3. Request volume (this month)
        const requestCount = await Request.countDocuments({
          "department.department_id": deptId,
          created_at: { $gte: startOfMonth, $lte: endOfMonth },
        });

        // 4. Payroll cost (this month)
        const payrollStats = await Payroll.aggregate([
          ...
          {
            $match: {
              "employee.department.department_id": deptId,
              month: currentMonth + 1,
              year: currentYear,
              status: { $in: ["Approved", "Paid"] },
            },
        ...
        const totalPayroll =
          payrollStats.length > 0 ? payrollStats[0].totalPayroll : 0;
        const totalOvertime =
          payrollStats.length > 0 ? payrollStats[0].totalOvertime : 0;
    ...
    comparisonData.sort((a, b) => b.employeeCount - a.employeeCount);
    ...
    res.status(500).json({
      message: "Lỗi khi so sánh phòng ban",
      error: error.message,
    });
  }
};
...
/**
 * GET /api/dashboard/stats/late-employees-today
 * Lấy danh sách nhân viên đi muộn hôm nay
 * @access Admin only
 */
exports.getLateEmployeesToday = async (req, res) => {
  try {
    ...
    const lateEmployees = await Attendance.find({
      date: today,
      status: { $in: ["Late", "Late & Early Leave"] },
    })
      .populate("userId", "full_name email avatar department")
      .select("clockIn lateMinutes")
      .sort({ lateMinutes: -1 })
      .lean();

    const formattedData = lateEmployees.map((att) => ({
      employeeId: att.userId?._id,
      employeeName: att.userId?.full_name || "Unknown",
      email: att.userId?.email,
      avatar: att.userId?.avatar,
      department: att.userId?.department?.department_name || "N/A",
      clockInTime: att.clockIn,
      lateMinutes: att.lateMinutes || 0,
      isSevere: att.lateMinutes > 30,
    }));

    res.status(200).json({
      total: lateEmployees.length,
      severe: formattedData.filter((e) => e.isSevere).length,
      employees: formattedData,
    });
  } catch (error) {
    console.error("❌ Error fetching late employees:", error);
    res.status(500).json({
      message: "Lỗi khi lấy danh sách nhân viên đi muộn",
      error: error.message,
    });
  }
};
...
/**
 * GET /api/dashboard/stats/manager-late-employees-today
 * Lấy danh sách nhân viên đi muộn hôm nay trong phòng ban
 * @access Manager only
 */
exports.getManagerLateEmployeesToday = async (req, res) => {
  try {
    const managerId = req.user._id;
    ...
    const lateEmployees = await Attendance.find({
      ...
      .populate("userId", "full_name email avatar")
      .select("clockIn lateMinutes")
      .sort({ lateMinutes: -1 })
      .lean();
