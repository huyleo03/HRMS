...
exports.checkDepartmentManager = async (req, res) => {
  try {
    ...
    if (department.managerId) {
      return res.status(200).json({ success: true, hasManager: true, message: "Phòng ban đã có quản lý" });
    }
    res.status(200).json({ success: true, hasManager: false, message: "Phòng ban chưa có quản lý" });
  }
  catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
