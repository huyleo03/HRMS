...
// ===== 10. GET ALL HOLIDAYS INCLUDING DELETED (Admin only - for audit) =====
exports.getAllHolidaysIncludingDeleted = async (req, res) => {
  try {
    console.log('🔍 [GET ALL WITH DELETED] Admin checking all holidays including deleted');
    ...
    // If no status filter, it will return ALL (Active + Inactive)
    
    const holidays = await Holiday.find(query)
      .populate("createdBy", "full_name email")
      .populate("updatedBy", "full_name email")
      .populate("departments", "department_name")
      .sort({ date: -1, status: 1 }); // Sort by date desc, then by status
    ...
    console.log(`✅ [GET ALL WITH DELETED] Found ${summary.total} holidays (Active: ${summary.active}, Deleted: ${summary.inactive})`);
    ...
    console.error("❌ [GET ALL WITH DELETED] Error:", error);
    res.status(500).json({
      success: false,
      message: error.message || "Lỗi khi lấy danh sách holidays",
    });
  }
};
