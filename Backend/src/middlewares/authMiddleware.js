...
exports.canManageEmployee = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'Manager') {
      ...
      if (employee.department.department_id?.toString() === req.user.department.department_id?.toString()) {
        return next();
      }
    }
...
exports.requireCompleteProfile = (req, res, next) => {
  if (!req.user.profileCompleted && req.user.role !== 'Admin') {
    return res.status(400).json({
      message: 'Vui lòng hoàn thành thông tin cá nhân trước khi sử dụng hệ thống.',
