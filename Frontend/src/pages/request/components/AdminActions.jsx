...
import { forceApproveRequest, forceRejectRequest } from "../../../service/RequestService";
...
const AdminActions = ({ request, onActionSuccess }) => {
  ...
  // Check if admin can take action on this request
  const canTakeAction = ['Pending', 'Manager_Approved', 'NeedsReview'].includes(request.status);
  ...
  const handleForceApprove = async () => {
    ...
    try {
      ...
      alert(error.message || "Có lỗi xảy ra khi phê duyệt đơn");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleForceReject = async () => {
    ...
    try {
      ...
      alert(error.message || "Có lỗi xảy ra khi từ chối đơn");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!canTakeAction) {
    return (
      <div className="admin-actions-disabled">
        ...
        <p>Không thể thao tác với đơn ở trạng thái hiện tại</p>
      </div>
    );
  }
  ...
  return (
    <div className="admin-actions">
      ...
      <p className="admin-actions-description">
        Bạn có thể phê duyệt hoặc từ chối đơn này bất kể quy trình phê duyệt hiện tại.
      </p>
        ...
        <div className="modal-overlay" onClick={() => setShowApproveModal(false)}>
          <div className="modal-contentt" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p>Bạn đang dùng quyền Admin để phê duyệt đơn này ngay lập tức, bỏ qua quy trình phê duyệt thông thường.</p>
